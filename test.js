...
global.window = {};
eval(src + ";globalThis.GEN = GEN; globalThis.CHAL = CHAL; globalThis._setLevel = v => { level = v; };");
let fails = 0, total = 0;
for (const lvl of [1, 3, 5]) {           // difficulty tiers 0, 1, 2
  ...
  for (const GENSET of [GEN, CHAL]) for (const mode of ["A","B","C"]) {
    for (let i = 0; i < 3000; i++) {
      const q = GENSET[mode]();
      total++;
      if (!q.opts.includes(q.a)) { console.log("MISSING ANSWER", lvl, mode, JSON.stringify(q)); fails++; }
